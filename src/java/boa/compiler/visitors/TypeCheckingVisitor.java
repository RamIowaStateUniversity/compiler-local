...
protected class FixPCheckingVisitor extends AbstractVisitorNoArg {
	...
	public void visit(final Program n, final SymbolTable env) {
		//System.out.println("stat size " +n.getStatementsSize());
		SymbolTable st;
	...
	public void visit(final Component n, final SymbolTable env) {
		...
		//System.out.println(n.getType().getClass());
	...
	/** {@inheritDoc} */
	/*@Override
	public void visit(final WhenStatement n, final SymbolTable env) {
		SymbolTable st;
		try {
			st = env.cloneNonLocals();
		} catch (final IOException e) {
	...
	/** {@inheritDoc} */
	/*@Override
	public void visit(final WhenExpression n, final SymbolTable env) {
		n.env = env;
		n.getType().accept(this, env);
		for (final Statement s : n.getBody().getStatements())
			if (!(s instanceof WhenStatement))
				throw new TypeCheckException(s, "only 'when' statements are allowed inside when bodies");
		whenChecker.start(n);
		n.getBody().accept(this, env);
		n.type = n.getType().type;
	}*/
	...
	/** {@inheritDoc} */
	/*@Override
	public void visit(final WhenType n, final SymbolTable env) {
		n.env = env;
		n.type = new BoaWhen();
	}*/
